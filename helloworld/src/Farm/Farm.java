...
public class Farm {
    ...
    public ArrayList<ArrayList<FarmObject>> getAllRows() {
        return farmObjects;
    }
    ...
    public int getRowSize(int row) {
        if (farmObjects != null && row >= 0 && row < farmObjects.size()) {
            return farmObjects.get(row).size();
        }
        return -1;
    }

    public boolean initFarm(int rows) {
    ...
    public ArrayList<FarmObject> getRow(int row) {
        if (farmObjects != null && row >= 0 && row < farmObjects.size()) {
            return farmObjects.get(row);
        }
        return null;
    }

    public boolean isInitialized() {
        return farmObjects != null;
    }

    public int getsize() {
        if (farmObjects != null) {
            return farmObjects.size();
        }
        return 0;
    }
    ...
    public boolean Save(String filePath) {
        if (farmObjects == null) {
            System.out.println("保存失败：农场未初始化");
            return false;
        }
